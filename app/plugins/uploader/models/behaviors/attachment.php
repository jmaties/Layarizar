...
/* Modificacion */
						$Model->data[$Model->alias][$attachment['dbColumn']] = Configure::read('Web.url').$basePath;
							...
							foreach ($attachment['transforms'] as $method => $options) {
								if (is_array($options) && isset($options['dbColumn'])) {
									...
									if ($path = $this->Uploader->$method($options)) {
										...
										$Model->data[$Model->alias][$options['dbColumn']] = Configure::read('Web.url').$path;
	...
	/* Borrando archivos */
	public function borra($file) {
		$imagen = strstr($file, 'files/base/');
		$this->Uploader->delete($imagen);
		$imagen2 = str_replace('_100x75','', $imagen);
		$this->Uploader->delete($imagen2);
		return true;
	}
